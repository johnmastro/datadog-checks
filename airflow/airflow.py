...
class AirflowCheck(AgentCheck):
    ...
    def get_dag_data(self, instance, tags):
        ...
        [count] = engine.execute(
            'select count(*) from dag where is_active = true'
        ).fetchone()
        ...
        self.gauge(
            '{}.dags_active.not_paused'.format(self.SOURCE_TYPE_NAME),
            active_and_not_paused_dags,
            tags=tags
        )
        self.gauge(
            '{}.dags_active.paused'.format(self.SOURCE_TYPE_NAME),
            count - active_and_not_paused_dags,
            tags=tags
        )
